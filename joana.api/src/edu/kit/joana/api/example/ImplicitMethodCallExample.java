 ...
 */
public class ImplicitMethodCallExample {
	...
	public static void main(String[] args) throws ClassHierarchyException, IOException, UnsoundGraphException,
		 ...
		 * to analyze
		 */
		JavaMethodSignature entryMethod = JavaMethodSignature.mainMethodOfClass("user.User");

		/**
		 * For multi-threaded programs, it is currently necessary to use the
		 * jdk 1.4 stubs
		 * Os stubs incluem modelos predefinidos de metodos nativos na J2SE
		 */
		...
		// for example: fields
		ana.addSourceAnnotation(program.getMethod("user.User.login(Luser/User;)V"), BuiltinLattices.STD_SECLEVEL_HIGH);
		ana.addSinkAnnotation(program.getMethod("user.User.toString()Ljava/lang/String;"),
