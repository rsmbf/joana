...
public class DirectMethodCallExample {
	...
	public static void main(String[] args) throws ClassHierarchyException, IOException, UnsoundGraphException, CancelException {
		...
		SDGSerializer.toPDGFormat(program.getSDG(), new FileOutputStream("/Users/rodrigoandrade/Dropbox/Temp/SDGDirect.pdg"));
		
		IFCAnalysis ana = new IFCAnalysis(program);
		/** annotate sources and sinks */
		// for example: fields
		// provavelmente ele so implementa o indireto
		ana.addSourceAnnotation(program.getMethod("Main.first()V"), BuiltinLattices.STD_SECLEVEL_HIGH);
		ana.addSinkAnnotation(program.getMethod("SpareMain.second(Ljava/lang/String;)V"), BuiltinLattices.STD_SECLEVEL_LOW);
